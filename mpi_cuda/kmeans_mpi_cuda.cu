...
// GPU kernel: assign each point to nearest centroid
...
int run_kmeans_mpi_cuda(const Config &cfg) {
  ...
  if (rank == 0) {
    ...
    std::cout << "K=" << k << "  max_iter=" << cfg.max_iter
              ...
              << "  mpi_ranks=" << size << "  block_size=" << block_size
              << "\n";
  }
  ...
  cudaMalloc(&d_pts, local_n * NUM_FEATURES * sizeof(double));
  cudaMalloc(&d_cents, k * NUM_FEATURES * sizeof(double));
  cudaMalloc(&d_labels, local_n * sizeof(int));

  // Copy points to device (once)
  cudaMemcpy(d_pts, local_pts, local_n * NUM_FEATURES * sizeof(double),
  ...
  for (; iter < cfg.max_iter; ++iter) {
    ...
    assign_kernel<<<grid_size, block_size>>>(d_pts, d_cents, d_labels, local_n,
                                             k);
    cudaDeviceSynchronize();

    // Copy labels back
    cudaMemcpy(local_labels, d_labels, local_n * sizeof(int),
